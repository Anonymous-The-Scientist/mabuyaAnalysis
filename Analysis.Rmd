---
title: "Deceptive density: evidence-oriented inference reveals how subsidies can mask predator death-traps (Appendix A)"
author: "Anonymous The Scientist"
date: "`r Sys.Date()`"
output:
  pdf_document:
    toc: true
...
This analysis examines Mabuya (Trachylepis atlantica; skink) populations across three distinct sites (i.e., LPU [Low Predator Unsubsidized], HPU [High Predator Unsubsidized] and HPS [High Predator Subsidized]) on Fernando de Noronha archipelago, investigating how invasive species presence and food supplementation affect population density, body size, and injury rates.
...
- **Main Island: 16.89 km²**
  ...
  - HPS: 5.466 km²
- **LPU: 1.33 km²**
- **TOTAL UNSUBSIDIZED (HPU + LPU): 12.754 km²**
- **TOTAL AREA (HPU + HPS + LPU): 18.22 km²**
...
pointsCounts[Site == "HPS", pointsArea := 5.466]
pointsCounts[Site == "HPU", pointsArea := 11.424]
pointsCounts[Site == "LPU", pointsArea := 1.33]
...
- **HPS**: 39% points over 30% area
- **HPU**: 37% points over 62% area  
- **LPU**: 25% points over 7% area

The sampling is reasonably proportional to area, with only modest over-sampling of LPU and slight under-sampling of HPU relative to area.
...
```{r create-predictors}
siteLevels <- factor(c("HPS", "HPU", "LPU"),
                       levels = c("HPS", "HPU", "LPU"))

# Invasive Species Presence (assuming key invasives like Cats and Tejus are largely absent from LPU)
mabuia_dd[Site == "HPS", InvasiveSpeciesPresence := 1]
mabuia_dd[Site == "HPU", InvasiveSpeciesPresence := 1]
mabuia_dd[Site == "LPU", InvasiveSpeciesPresence := 0]

# Food Supplementation
mabuia_dd[Site == "HPS", FoodSupplementation := 1]
mabuia_dd[Site == "HPU", FoodSupplementation := 0]
mabuia_dd[Site == "LPU", FoodSupplementation := 0]
...
density_df$Site <- ifelse(
  ...
  ifelse(
    density_df$InvasiveSpeciesPresence == 0 & density_df$FoodSupplementation == 0, "LPU",
    ifelse(
      density_df$InvasiveSpeciesPresence == 1 & density_df$FoodSupplementation == 1, "HPS",
      "HPU"
    )
  )
)

density_df$Site <- factor(density_df$Site, levels = c("Hypothetical Site", "LPU", "HPS", "HPU"))
...
```{r density-plot}
two_line_labels <- c(
  "Hypothetical LPS\n(Low Predator / Subsidised)",
  "LPU\n(Low Predators/ Unsubsidised)",
  "HPS\n(High Predators/ Subsidised)",
  "HPU\n(High Predators/ Unsubsidised)"
)
...
density_plot <- ggplot(density_df, aes(x = Site, y = mean_density, fill = Site)) +
  ...
  # Add a bracket connecting HPS and HPU bars
  annotate("segment", x = "HPS", xend = "HPU", y = 0.40, yend = 0.40,
           color = "black", linewidth = 0.7) +
  # New position for the left vertical tick (pointing down).
  annotate("segment", x = "HPS", xend = "HPS", y = 0.40, yend = 0.38,
           color = "black", linewidth = 0.7) +
  # New position for the right vertical tick (pointing down).
  annotate("segment", x = "HPU", xend = "HPU", y = 0.40, yend = 0.38,
           color = "black", linewidth = 0.7) +
  # New position for the text, slightly above the bracket line.
  annotate("text", x = 3.5, y = 0.43, label = "CMR confirms HPS > HPU (p < 0.001)",
...
This table shows us that alpha (resighting rate/detection probability) is significantly **HIGHER** in HPU while U (population size) is significantly **LOWER** in HPU. So, this means that we DO see/capture more in HPU, and yet, the population size is considerably smaller. This is likely due to pressure of invasive species and the potential effects of food supplementation.

**Note**: We did not manage capture-recapture on LPU islands due to impossible logistics.
...
pointsCounts_size[site == "HPS", pointsArea := 5.466]
pointsCounts_size[site == "HPU", pointsArea := 11.424]
pointsCounts_size[site == "LPU", pointsArea := 1.33]
...
- **HPS**: 47% points over 30% area
- **HPU**: 36% points over 62% area
- **LPU**: 16% points over 7% area

## Step 3: Factor Level Setup

```{r size-factors}
mabuia_ds[, site := factor(site, levels = c("LPU", "HPS","HPU"))] # "LPU" as reference
...
Autotomy was proportionally more frequent in the HPS than in HPU and roughly twice that observed on the secondary islands, suggesting elevated predation pressure on the main island. However, these differences were not statistically significant. Our power analyses indicate that this outcome is largely due to limited sample sizes—particularly on the secondary islands—rather than the absence of a biological effect, highlighting the need for expanded sampling to better resolve these patterns.
...
dt_df$site <- factor(dt_df$site, levels = c("LPU", "HPS", "HPU"))
...
autotomy_df_final$site <- factor(autotomy_df_final$site, levels = c("LPU", "HPS", "HPU"))
